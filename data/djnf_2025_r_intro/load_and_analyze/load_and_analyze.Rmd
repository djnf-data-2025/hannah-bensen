...
1. How does yearly apprehensions vary seasonally? 

```{r}

df <- bp_fy2020 %>% 
  select(-c(fy, yearly_total)) %>% 
  pivot_longer(cols = -c(sector)) %>% 
  mutate(
    season = case_when(
      name %in% c("january", "february", "march") ~ "winter", 
      name %in% c("april", "may", "june") ~ "spring",
      name %in% c("july", "august", "september") ~ "summer",  
      name %in% c("october", "november", "december") ~ "fall", 
      TRUE ~ NA_character_
    )
  ) %>% 
  group_by(season) %>% 
  summarize(seasonal_apprehensions = sum(value, na.rm = TRUE)) %>% 
  arrange(desc(seasonal_apprehensions))
